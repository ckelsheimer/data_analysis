function [pfit,err,gfstruct] = globalFit_cjk(dataMatrix,w1,w3,p_array,gfstruct,lb,ub)
...
err_fun = @(p,gfstruct) sum(sum(sum((dataMatrix-analyticalResponseFunctionsFun_cjk_mainband(p,w1,w3,gfstruct).*scale).^2)))./maxScale;
