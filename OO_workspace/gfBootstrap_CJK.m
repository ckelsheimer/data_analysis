function [pboot,gfstruct] = gfBootstrap_CJK(dataMatrix,w1,w3,p0,gfstruct,lb,ub,nboot)

scale = gfScale(dataMatrix);
maxScale = abs(min(min(min(dataMatrix))));
err_fun = @(p,gfstruct) sum(sum(sum((dataMatrix(gfstruct.bootstrap)-analyticalResponseFunctionsFun_cjk_mainband(p,w1,w3,gfstruct).*scale(gfstruct.bootstrap)).^2)))./maxScale;
